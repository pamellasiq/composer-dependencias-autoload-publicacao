...
usando o `PHPUnit`, uma ferramenta que nos ajuda a garantir que nosso código funciona como esperado.

Vimos como criar um teste que simula uma requisição HTTP, garantindo que o HTML retornado seja processado corretamente. Para isso, usamos um "dublê" de testes, chamado Mock, que nos permite controlar o comportamento do cliente HTTP, sem precisar fazer uma requisição real.

Também aprendemos como executar o teste no PhpStorm e na linha de comando, e como podemos fazer o teste falhar propositalmente para entender como ele funciona.

## Padrão de Codificação

O PHPCS é como um "fiscal de código", que verifica se você está seguindo as regras de estilo de código, como a PSR12. Ele te avisa se você está usando a sintaxe do PHP de forma errada, se está organizando o código de forma confusa ou se está usando funções de forma inadequada.

Com o PHPCS, você pode ter certeza de que seu código está limpo, fácil de ler e de manter, como um bom projeto de construção!

Você pode instalar o PHPCS no seu projeto usando o comando

``` sh
composer e
```

Depois, usar o comando phpcs para analisar seu código.

``` sh
phpcs --standard=PSR12 src\
```

Use phpcs --help para verificar os comandos disponíveis

Saber mais sobre [PSR12](https://www.php-fig.org/psr/psr-12/)

## Para encontrar erros no código

Ferramenta PHAN, usamos o comando 
```sh
composer require --dev phan/phan
```

O Phan é uma ferramenta de desenvolvimento, por isso usamos o `--dev`.

O Phan precisa de um arquivo de configuração para saber quais diretórios analisar e quais ignorar. Criamos um arquivo `config.php` na pasta `.phan` e configuramos os diretórios do nosso projeto.

O Phan nos ajuda a encontrar erros que podem passar despercebidos, como tipos de variáveis não declarados e métodos não existentes. Ele é uma ferramenta poderosa para garantir a qualidade do código.